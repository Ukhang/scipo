...
export const Planet: React.FC<PlanetProps> = ({
  ...
  const colors = useMemo(() => ({
    baseColor: new THREE.Color().setHSL(0.05 + vegetation * 0.25, 0.6 - vegetation * 0.2, 0.3 + vegetation * 0.2),
    waterColor: new THREE.Color().setHSL(0.6, 0.7, 0.3 + waterLevel * 0.3),
    atmosphereColor: new THREE.Color().setHSL(0.05 + atmosphereDensity * 0.55, 0.7, 0.5),
    cloudOpacity: Math.min(atmosphereDensity * 0.8, 0.7),
    iceColor: new THREE.Color(0xffffff),
  ...
  useFrame((_, delta) => {
    ...
    if (waterRef.current) waterRef.current.rotation.y += delta * (rotationSpeed * 0.8)
    if (iceCapsRef.current) iceCapsRef.current.rotation.y += delta * rotationSpeed
  })
  ...
  const terrainGeometry = useMemo(() => {
    ...
    for (let i = 0; i < positions.length; i += 3) {
      const vertex = new THREE.Vector3(positions[i], positions[i + 1], positions[i + 2]).normalize()
      const mountainNoise =
      ...
      for (let c = 0; c < 8; c++) {
        ...
        if (distToCrater < craterSize) craterNoise -= (craterSize - distToCrater) * 0.5 * craterFactor
      }

      const roughnessFactor = Math.max(1 - terraformingStage * 0.7, 0.3)
      const scale = 10 * (1 + (mountainNoise + craterNoise) * roughnessFactor)
  ...
  const iceCapsGeometry = useMemo(() => {
    ...
    for (let i = 0; i < positions.length; i += 3) {
      const vertex = new THREE.Vector3(positions[i], positions[i + 1], positions[i + 2]).normalize()
      const minDist = Math.min(...poles.map((pole) => vertex.distanceTo(pole)))
      const iceThreshold = 0.7 - iceFactor * 0.5
      const iceValue = minDist < iceThreshold ? 1 - minDist / iceThreshold : 0
      const finalIceValue = Math.max(0, Math.min(1, iceValue + Math.sin(vertex.x * 30) * Math.sin(vertex.z * 30) * 0.1))
      polarMask[i / 3] = finalIceValue

      if (finalIceValue > 0) {
        const raiseFactor = 1 + finalIceValue * 0.05
        positions[i] *= raiseFactor
        positions[i + 1] *= raiseFactor
        positions[i + 2] *= raiseFactor
      }
    }
  ...
  const iceShader = useMemo(() => ({
    uniforms: { iceCapsColor: { value: colors.iceColor }, iceFactor: { value: Math.max(0, 1 - temperature * 2) } },
    vertexShader: `
    ...
    <group>
      ...
      {iceCapsGeometry && (
        <mesh ref={iceCapsRef} geometry={iceCapsGeometry}>
          <shaderMaterial {...iceShader} transparent depthWrite={false} />
        </mesh>
      )}
